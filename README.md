...
3. Configure the Viessmann `username`, `password`, and `client_id` in the app configuration.
4. The app will automatically use the Home Assistant MQTT service credentials. Advanced users can override them in YAML mode using `mqtt_uri`, `mqtt_username`, and `mqtt_password`.
